...
public class ModData {
	public string name, title, parent_game;
    ...
    public ArrayList parent_candidates;

    public ModData(string load_me, ArrayList from_here, PathHandler paths, SteamHandler steam)
    {
        FileInfo xml_file = new FileInfo(load_me);
        file_holder add_me;
        parent_candidates = from_here;
			    ...
			    if(read_me.NodeType==XmlNodeType.Element) {
				    switch (read_me.Name) {
					    case "title":
						    ...
						    break;
                        case "depend":
                            read_me.MoveToAttribute("game");
                            parent_game = read_me.Value;
                            break;
                        case "root":
...
//            string check_path = path.Replace("%PROGRAMFILES%",Environment.GetEnvironmentVariable("PROGRAMFILES"));
    ...
    public bool detect(PathHandler paths, SteamHandler steam) {
        ...
        foreach(file_holder parent in parent_candidates) {
            foreach (string root in roots) {
                if (root.StartsWith("%STEAM")){
                    ...
                    if(parent.relative_path.StartsWith("%STEAM")) {
                        detected_roots.AddRange(steam.getSteamPaths(parent.relative_path + "\\" + root));
                    } else {
                        detected_roots.AddRange(paths.getPath(parent.relative_path + "\\" + root, override_virtualstore));
                    }
                }
            }
        }
