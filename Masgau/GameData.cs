                            ...
                            while(read_me.MoveToNextAttribute()) {
                                switch(read_me.Name) {
                                    ...
                                    case "windows_version":
                                        new_root.windows_version= read_me.Value;
                                        break;
                                }
                            }
                            roots.Add(new_root);
...
//            string check_path = path.Replace("%PROGRAMFILES%",Environment.GetEnvironmentVariable("PROGRAMFILES"));
    ...
    public void detect(PathHandler paths, SteamHandler steam, playstationHandler playstation, Dictionary<string, string> game_profiles) {
        ...
        foreach (root_holder root in roots) {
			...
			if(root.windows_version==null||root.windows_version==windows_version) {
				...
				if(root.registry!=null) {
					...
					if(registry_sub_key!=null) {
						try {
							...
							// TODO: Error cathing for mistyped registry keys
						}
					}
				}
				...
				if(root.game!=null) {
					if(game_profiles.ContainsKey(root.game)){
						parent_game = new GameData(game_profiles[root.game],paths,steam,playstation,game_profiles,platform);
						...
						MessageBox.Show("The specified parent game " + root.game + " for " + title + "is not present in the profiles xml. You probably spelled it wrong.","Wasting my gorramn time",MessageBoxButtons.OK,MessageBoxIcon.Error);
					}
				}
			}
		...
		if(psp_ids.Count>0) {
			if(playstation.psp_saves!=null) {
				foreach(playstation_id psp_id in psp_ids) {
					...
					if(found_id!=null) {
						add_me.relative_path = Path.Combine(playstation.psp_saves.Substring(3),found_id+"*");
		...
		if(ps3_ids.Count>0) {
			if(playstation.ps3_saves!=null) {
				foreach(playstation_id ps3_id in ps3_ids) {
					...
					if(found_id!=null) {
						add_me.file_name = null;
						add_me.relative_path = Path.Combine(playstation.ps3_saves.Substring(3),found_id+"*");
    ...
    public ArrayList interpretPath(string interpret_me, SteamHandler steam, PathHandler paths) {
        ...
        if(steam.installed&&interpret_me.ToLower().StartsWith(Path.Combine(steam.path.ToLower(),"steamapps\\common"))) {
            ...
            foreach(KeyValuePair<string,user_data> check_me in paths.users) {
                if(check_me.Value.user_dir!=null&&interpret_me.ToLower().StartsWith(check_me.Value.user_dir.ToLower())) {
                    return_me.AddRange(paths.getPath("%USERPROFILE%" + interpret_me.Substring(check_me.Value.user_dir.Length),override_virtualstore));
                }
            }
					    ...
					    if(Directory.Exists(root.absolute_path)) {
                            ...
                            foreach(string file_name in files) {
                                if(Path.GetDirectoryName(file_name).Length==add_me.absolute_root.Length)
                                    ...
                                    add_me.absolute_path = Path.GetDirectoryName(file_name).Substring(add_me.absolute_root.Trim(Path.DirectorySeparatorChar).Length+1);
                    ...
                    if(Directory.Exists(root.absolute_path)) {
                        foreach(FileInfo read_me in new DirectoryInfo(root.absolute_path).GetFiles(save.file_name)) {
                            if (read_me.DirectoryName.Length == add_me.absolute_root.Length)
                                ...
                                add_me.absolute_path = read_me.DirectoryName.Substring(add_me.absolute_root.Trim(Path.DirectorySeparatorChar).Length + 1);
