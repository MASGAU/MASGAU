    ...
    public class AMonitorProgramHandler<L>: AProgramHandler<L> where L: ALocationsHandler
    {
        ...
        private void synchronize() {
            ...
            foreach (GameHandler game in Core.games)
			{
				...
				if (!game.sync_available||!game.detected||!game.sync_enabled)
                    continue;
                ...
                if(game.detected_locations.Count==0) {
                    ...
                    if(game.has_sync_location) {
				        foreach (KeyValuePair<string,DetectedLocationPathHolder> game_root in game.detected_locations){
                            if(game_root.Value.full_dir_path==game.sync_location) {
                                if(ArchiveHandler.canWrite(new DirectoryInfo(game.sync_location))) {
                    ...
                    while(retry) {
                        ...
                        if(reply.cancelled) {
                            ...
                            if(ArchiveHandler.canWrite(new DirectoryInfo(reply.selected_option))) {
        ...
        public int countMonitoredGames() {
            int return_me = 0;
            foreach(GameHandler game in Core.games) {
                if(game.backup_enabled)
                    return_me++;
        ...
        private void backupLoop() {
            ...
            while(!CancellationPending) {
                ...
                if(file.change_type== WatcherChangeTypes.Changed||file.change_type== WatcherChangeTypes.Created||file.change_type== WatcherChangeTypes.Renamed) {
                    ...
                    lock (paths_to_games) {
                        foreach (GameID game in paths_to_games[file.root]) {
                            if(!Core.games.all_games.get(game).backup_enabled)
                                continue;

						    List<DetectedFile> these_files = Core.games.all_games.get(game).checkThis(Path.Combine(file.root, file.path));
                            ...
                            foreach (DetectedFile this_file in these_files) {
                                ...
                                ArchiveHandler archive = archives.get(game, this_file.owner, this_file.type);

                                try {
                                    if (archive == null) {
                                        if (this_file.owner == null)
                                            archive = new ArchiveHandler(Core.settings.backup_path, new ArchiveID(game, null, this_file.type));
                                        else
                                            archive = new ArchiveHandler(Core.settings.backup_path, new ArchiveID(game, this_file.owner, this_file.type));
                                        archives.Add(archive);
                                    }
                                    //monitorNotifier.ShowBalloonTip(10, "Safety Will Robinson", "Trying to archive " + file.path, ToolTipIcon.Info);
                                    Communication.Message.MessageHandler.suppress_messages = true;
                                    ...
                                    Communication.Message.MessageHandler.suppress_messages = false;
                                }
                    ...
                    if(!File.Exists(file.full_path)&&file.change_type!= WatcherChangeTypes.Deleted)
                        continue;
                    GameHandler game = getGameHandler(file.root);
                    if(game==null||!game.backup_enabled||!sync_paths.ContainsKey(game.id))
                        continue;
